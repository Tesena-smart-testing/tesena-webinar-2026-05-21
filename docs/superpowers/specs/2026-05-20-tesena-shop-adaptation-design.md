...
**`acc.ts`** and **`int.ts`** — rename key, use demo credentials:
...
1. **Test credentials** — `demo@prestashop.com` / `prestashop_demo` (shared demo account, same for both environments).
