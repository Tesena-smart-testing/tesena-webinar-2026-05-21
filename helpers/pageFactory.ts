 ...
 */
export async function expectPage<T extends { expectLoaded(): Promise<void> }>(page: Page, t: Texts, Ctor: LoadablePage<T>): Promise<T> {
 ...
 * @returns novou instanci vytvořené page
 */
export async function gotoPage<T extends { expectLoaded(): Promise<void>, goto(): Promise<void> }>(page: Page, t: Texts, Ctor: LoadablePage<T>): Promise<T> {
    const po = new Ctor(page, t);
    await po.goto();
    await po.expectLoaded();
    return po;
